...
bool add_new_macro(macro_table *macro_table, char *name) {
    ...
    *new_macro = macro_init(name);
    ...
    if (macro_table->macros_array[index] == NULL) {
        ...
        /* Append the new macro to the end of the linked list*/
    ...
    return TRUE; /* Return TRUE when the macro is successfully added*/
}
...
bool remove_symbol(symbol_list *table, char *key) {
    ...
    while (current_symbol) {
        if (strcmp(current_symbol->name, key) == 0) {
            ...
            free(current_symbol->name); /* Free the memory for the name */
            free(current_symbol); /* Free the memory for the symbol */
...
void free_symbol_list(symbol_list *table){
    ...
    while (current_symbol){
        prev_symbol = current_symbol;
        current_symbol = (symbol *)current_symbol->next;
        free(prev_symbol);
    }
    free(table);
}
