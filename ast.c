...
int parse_single_oprand(char *args, char *error_msg, instruction_word *tmp_word);
bool add_to_inst_list(inst_list *list, instruction_word *ins);
...
int parse_single_oprand(char *args,char *error_msg,instruction_word *tmp_word ){
    int num = 0;    
    immediate_direct_word *rand_word = NULL;
    register_word *reg_word = NULL;    
...
bool add_to_inst_list(inst_list *list, instruction_word *instruction){
    inst_node *new_node = (inst_node *)malloc(sizeof(inst_node));
    if (new_node == NULL){
        printf("Error: unable to allocate memory\n");
        return FALSE;
    }
    new_node->inst = instruction;
    new_node->next = NULL;
    if (list->head == NULL){
        list->head = new_node;
        list->tail = new_node;
